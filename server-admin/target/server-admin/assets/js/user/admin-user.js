...
userModule.factory('PostIP',[function(){
  var postIP = {
    ...
    getdetail:"http://192.168.0.108:8000/item/render",
...
userModule.controller('feedbacklistController', ["$scope", "feedback", "$http","PostIP",
	function($scope, feedback, $http,PostIP) {
		...
		$scope.getfeedbacklist = function() {
		    progress.start();
			$http.post(PostIP.getfeedbacklist, "page=" + $scope.page).success(function(data) {
...
userModule.controller('goodslistController', ["$scope", "$http","PostIP",
	...
	progress.start();
			$http.post(PostIP.getgoodslist, "page=" + $scope.page).success(function(data) {
...
userModule.controller('activitylistController',['$scope','$http','PostIP',function($scope,$http,PostIP){
  var progress = $.AMUI.progress;
        ...
        progress.start();
        			$http.post(PostIP.getactivitylist, "page=" + $scope.page).success(function(data) {
        		...
        		$scope.deleteactivity = function(id) {
        			$http.post(PostIP.deleteactivity, 'activity_id=' + id).success(function(data) {
        				if (data.result == "success") {
        					$scope.goods.forEach(function(item, index, array) {
        						if (item.item_id == id) {
        							array.splice(index, 1);
